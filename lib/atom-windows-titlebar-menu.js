...
export default class AtomWindowsTitlebarMenu {
   ...
   listenForMenuEvents(){
      ...
      // Patch to only open if just alt key
      var keys = 0
      document.body.addEventListener('keydown', function(e) { if(!e.repeat) keys ++ })
      window.addEventListener('blur', function(){ keys = 0 })

      document.body.addEventListener('keyup', function(e) {
         keys -= 1
         if(keys == 0 && e.code == 'AltLeft') that.container.classList.toggle('show')
      })
   }
